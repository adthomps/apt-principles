...
Use this prompt to update agent instructions, skills, prompts, and repo operating rules while preserving the ownership split between `apt-principles` and `apt-agent-standards`. It supports agent guidance quality, AI readiness, and cross-tool consistency without turning this doctrine repo into the installer.
...
- apt-principles/context-packs/apt-agent-pack.md
- apt-principles/checklists/ai-agent-review-checklist.md
- apt-principles/references/agent-standards-contract.json
...
2. Identify whether the change belongs in apt-principles doctrine, apt-agent-standards distribution, or target-repo local context.
...
5. Residual risks and follow-up for apt-agent-standards when needed.
```
...
- Do not merge installer scripts, profile detection, path mapping, or managed sync behavior into `apt-principles`.
