...
Review an APT Agent or `apt-agent-standards` implementation against canonical APT Principles without copying implementation catalogs into `apt-principles`. Use this when harness agents, routing docs, context packs, manifests, lifecycle scripts, or tool-native distribution files change.
...
- Any proposed changes to `apt-principles` doctrine, standards, prompts, examples, or references.
...
5. Boundary violations, if any, between apt-principles, apt-agent-standards, and target repositories.
...
- Do not copy agent catalogs, routing matrices, manifests, or scripts into `apt-principles`.
- Do not treat model names in implementation files as APT doctrine.
- Do not recommend moving installer, sync, scan, repair, profile detection, or tool-native mapping behavior into `apt-principles`.
