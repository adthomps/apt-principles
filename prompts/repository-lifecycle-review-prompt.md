...
- Current `README.md`, `AGENTS.md`, `.agent-standards.json`, and `docs/project-context.md` status when present.
...
8. Ownership: Does `apt-principles` remain doctrine/build-kit/reference only while `apt-agent-standards` owns installer and distribution behavior?
...
- Do not recommend adding installer, profile detection, path mapping, or `.agent-standards.json` ownership to `apt-principles`.
